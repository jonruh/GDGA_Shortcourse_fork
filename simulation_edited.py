...
class Simulation(Object):
    ...
    *kwargs
    ...
    def _set_simulation_values(self, model, perturbations, verbose=False):
        ...
        reset = False
        try:
            print('try setting perturbations')# <-- added
            for key, value in iteritems(perturbations):
                # Perturb value to a number if value is float
                if isinstance(value, float):
                    sim_values_to_set[key] = value
                    print('Set a value according to perturbations')# <-- added
                    continue
                ...
                try:
                    ...
                    reset = True
                    print('reset = true in perturbations')# <-- added
    ...
    def _set_values_in_roadrunner(self, model, reset, sim_values_to_set):
        ...
        rr = self.roadrunner
        if reset:
            print('Went into reset = true if loop of _set_values_in_roadrunner')# <-- added
        ...
        # changed on 21/05/2023 by jonruh:
        # changes start here
        else:
            print('Went into reset = False if loop of _set_values_in_roadrunner')# <-- added
            ...
            for key, value in iteritems(sim_values_to_set):
                ...
                rr.setValue(_make_init_cond(key), value)
            print('Finished for loop of _set_values_in_roadrunner')# <-- added
                # else:
